...
## YOUR MISSION:
Execute mobile web test cases for Sunbird Ed website in mobile view mode using Chrome browser on Android Emulator. The website UI matches the mobile app design.
...
## MOBILE WEB TESTING SPECIFIC GUIDANCE:

### Browser vs Native App Differences:

**You are testing a WEBSITE in mobile view, not a native app. Key differences:**

1. **Element Location:**
   - Use CSS Selectors as first choice (faster, more reliable for web)
   - XPath for complex web elements
   - No resource-id or accessibility-id (those are native app only)
   - Check for `<button>`, `<div>`, `<input>` HTML elements

2. **Navigation:**
   - Browser back button vs. app back button
   - Page URL changes (verify correct page by URL)
   - Page reloads may occur (wait for page load complete)
   - Handle browser chrome (address bar, etc.)

3. **Interactions:**
   - Click = tap on web elements
   - Scroll using JavaScript: `driver.execute('window.scrollTo(0, 500)')`
   - Forms use web input fields (check for HTML5 validation)
   - Dropdowns are `<select>` elements

4. **Waits:**
   - Wait for page load: `driver.waitUntil(() => driver.execute('return document.readyState') === 'complete')`
   - Wait for elements: Standard WebDriverIO waits work
   - AJAX calls may cause delays (wait for loading spinners to disappear)

5. **Common Web Issues:**
   - Cookie consent banners (dismiss them)
   - Browser notifications (allow/block as needed)
   - Viewport size (ensure mobile view, not desktop)
   - Page zoom (ensure 100% zoom)
   - Cache issues (may need to clear cache/cookies)

6. **Verification:**
   - Check page URL to confirm navigation
   - Check page title for context
   - Use browser console logs for debugging
   - Screenshots work same as native app

### Mobile Web Troubleshooting:

**If element not found on web:**
1. Inspect element in browser DevTools (Chrome remote debugging)
2. Verify mobile viewport is set correctly (not showing desktop version)
3. Check if element is in iframe (switch to iframe first)
4. Wait for AJAX/API calls to complete (network tab)
5. Check if JavaScript loaded properly (console errors)
...
| File | Action | Change |
|------|--------|--------|
| `ai_tester_stage_setting.md` | UPDATE | Change "mobile app" to "mobile web", add mobile web testing section |
| `TC_56_simple.txt` | REPLACE | Change "launch app" to "launch Chrome + navigate to URL" |
| `appium_mobile_web_config.txt` | CREATE NEW | Add Appium web browser configuration |

---

## **STEP-BY-STEP WHAT TO DO:**

1. ✅ Open `ai_tester_stage_setting.md`
   - Change mission statement (line ~10)
   - Add mobile web testing section (after core principles)
   
2. ✅ Open `TC_56_simple.txt`
   - Replace entire content with mobile web version above
   
3. ✅ Create new file `appium_mobile_web_config.txt`
   - Add the configuration content above
   
4. ✅ Ask your guide for:
   - Exact website URL (staging or production)
   - Any login differences (website vs app)
   - Any known UI differences between web and app

5. ✅ Test with Copilot:
```
   #file:ai_tester_stage_setting.md
   #file:appium_mobile_web_config.txt
   #file:TC_56_simple.txt
   
   Generate WebdriverIO test for mobile web (Chrome browser on Android emulator).
   Website URL: [insert URL from your guide]
...
**NOTE: Testing on mobile web browser, not native app. Use browser-specific strategies.**

### 3. ADAPTIVE ELEMENT LOCATION
```

**Find references to "app" and add clarifications:**
- Search for "app crash" → Change to "browser crash or page freeze"
- Search for "app responsive" → Change to "page responsive"

---

### **2. UPDATE: `TC_56_simple.txt`**

**Replace the ENTIRE file with:**
```
# TC_56: Course Progress Sync Verification (Mobile Web)

## Test Objective
Verify kebab menu appears at 100% course completion and "Sync progress now" option is enabled and functional on mobile web.

---

## Test Steps

Start an Appium session and launch Chrome browser on an Android Emulator.

Navigate to the Sunbird Ed website URL in mobile view.

Click on Login/Sign-in button.

Login with valid credentials (email: user1@yopmail.com, password: User1@123).

Navigate to "My Courses" and select an enrolled course.

Consume all course content to reach 100% completion.

Verify "You have successfully completed this course" message appears.

Locate and click the kebab menu (3 dots) in the course progress section.

Verify "Sync progress now" option is enabled (not grayed out).

Click on "Sync progress now" option.

Verify success message appears (toast notification or confirmation).
...
## Expected Results
- Kebab menu is visible at 100% completion
- "Sync progress now" option is enabled and clickable
- Sync operation executes successfully with confirmation message

---

After completing the above steps, write all these steps as an E2E test case using Appium & WebdriverIO for mobile web testing.
```
...
**Create this file with Appium configuration for mobile web:**
```
# Appium Configuration for Mobile Web Testing

## Desired Capabilities:

platformName: Android
platformVersion: 11 (or your emulator version)
...
- Set mobile view viewport: 375x667 (iPhone size) or 360x640 (Android size)
- Enable responsive design mode
- User agent: Set to mobile browser
- Disable desktop mode

## Website URL:
Replace with actual Sunbird Ed portal URL (e.g., https://test.sunbirded.org/ or production URL)

## Key Differences from Native App:
- Use browser element locators (CSS selectors, XPath for web elements)
- Handle page loads instead of activity loads
- Check for browser-specific popups (cookies, notifications)
- Scrolling uses JavaScript or browser scroll
- No native app permissions (camera, location) unless web prompts
